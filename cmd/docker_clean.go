...
If no services are specified, all services are cleaned.`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if len(args) > 0 {
			...
			availableServices, err := docker.GetServices()
			if err != nil {
		...
		if err := docker.RunCommand(rmArgs...); err != nil {
		...
		if err := docker.RunCommand(buildArgs...); err != nil {
		...
		if err := docker.RunCommand(upArgs...); err != nil {
